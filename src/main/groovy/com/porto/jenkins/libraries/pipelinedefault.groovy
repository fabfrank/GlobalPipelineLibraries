	...
	stages {
		stage("Checkout RTC") {
			...
			steps {
				...
				script {
					if (EXCLUDECOMPONENTS.equals('NONE')) {
						checkout([$class: 'RTCScm', avoidUsingToolkit: false, buildTool: 'jazz', buildType: [buildStream: "${STR}", clearLoadDirectory: true, 
						...
						checkout([$class: 'RTCScm', avoidUsingToolkit: false, buildTool: 'jazz', buildType: [buildStream: "${STR}", clearLoadDirectory: true, 
							componentLoadConfig: 'excludeSomeComponents', componentsToExclude: "${EXCLUDECOMPONENTS}", customizedSnapshotName: '', 
			...
			post {
				...
				always {
					script {
						EMAIL_REPORT=env.EMAIL_REPORT + ", " + propValue('email')
		...
		stage("Build Project") {
			...
			steps {
				dir("${ARTHEFACT}") {
					script {
						...
						if (env.SKIPTEST) {
							sh "mvn clean -DskipTests -U ${BUILDPROFILE} install"
						} else {
							sh "mvn clean -Dmaven.test.failure.ignore -U ${BUILDPROFILE} install"
						}
					}
				}
			}
			...
			post {
                ...
                changed {
                    notifyEmail(env.EMAIL_REPORT, env.CHANGELOGS)
                }
				always {
					dir("${ARTHEFACT}") {
						script {
			...
			steps {
				script {
					...
					if (TIPOCOMPONENTE.equals('ear')) {
					    ...
					    sh "curl -sS ${BUILD_URL}artifact/target/${MODULOEARNAME}-${pom.version}.${TIPOCOMPONENTE} -o ${SHAREDLIBNAME}/${MODULOEARNAME}-${pom.version}.${TIPOCOMPONENTE} -v -R --location-trusted --fail"
					}
				}
			}
			post {
